...
export default class App extends React.Component {
    ...
    delete(item) {
        let status;
        let authorization = localStorage.getItem("token_type") + " " + localStorage.getItem("access_token");
        fetch(BASE_URL + `/api/officers/${item.id}`, {
            method : "DELETE",
            headers: {
                "Accept"       : "application/json",
                "Authorization": authorization,
            ...
            .then(jsonData => {
                if (status === 200) {
                    this.setState(prevState => {
                        const officers = [...prevState.officers];
                        officers.splice(officers.indexOf(item), 1);
                        return {...prevState, officers};
                    });
                }
            });
    }
                        ...
                        <Grid item xs={11} sm={10} md={10} lg={10}>
                            ...
                            <TableContainer component={Paper}>
                                <Table aria-label="simple table">
                                    ...
                                    <TableBody>
                                        {this.state.officers.map((item, key) => {
                                            return (
                                                <TableRow key={key}>
                                                    ...
                                                    <TableCell className={`t-center f-small`}>
                                                        ...
                                                        <Link to={`/admin/officer/edit/${item.id}`}>
                                                            <Edit />
                                                        </Link>
                                                    </TableCell>
                                                </TableRow>
